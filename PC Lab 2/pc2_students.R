...
plot(ecdf(y_2006), xlab = "Spending in US-Dollars", sub = "(Truncated at 20,000 US-Dollars)", ylab = "cdf", main = "Distribution of Spending", xlim= c(0,20000))
...
# Prediction
fit <- predict(????, newdata = ????)$predictions
...
for (t in sizes){
  ...
  auc[auc[,1]== t,2] <- 1- mean(((log_y_2006[-training_set,])-fit)^2)/var(log_y_2006[-training_set,]) # store R-squared
}
...
fit_new <- predict(????, newdata = ????)$predictions
